...
class MockPlanner:
    ...
    def __call__(
        ...
        if tool_result is None:
            return {
                "thought": (
                    "Calculer le total des ventes avec "
                    "l'outil statistique."
                ),
...
""",
        ...
        return {
            "thought": "Le résultat nécessaire est disponible.",
            "tool_name": None,
            "python_code": None,
            "final_answer": (
...
class LLMPlanner:
    """
    Planner utilisant un modèle externe.

    Il décide soit :
    - d'appeler un outil ;
    - de produire une réponse finale.

    Le champ `thought` contient uniquement une justification
    courte de l'action choisie.
    """
...
"""
        ...
        if tool_name is not None:
            if tool_name not in ALLOWED_TOOLS:
                raise ValueError(
                    f"Planner selected forbidden tool: {tool_name}"
                )

            if not decision.get("python_code"):
                raise ValueError(
                    "Planner selected a tool without Python code"
                )

        elif not final_answer:
            raise ValueError(
                "Planner returned neither a tool nor a final answer"
            )
...
def get_planner() -> Planner:
    ...
    raise ValueError(
        f"Unsupported AGENT_PLANNER: {planner_type}"
    )